...
I would try to approach this problem by 1st trying to set an amount of data each seeder will send. 
the seeder will connect to the peer and upload it part of the data. after one of the seeders is done sharing its part of the data, the following seeder will connect and share it part of the data. 

or we can have the seeder upload the data to the sever and then send send it to the other cleint but at this point this is not p2p

according to class the way data is sent in p2p is framenting the data into small fragments. the number of fragments is determine by  = (datagram size/MTU). this small framents contain
the legth, ID, Fragflag, offset.

oh oh i think the connections are menaged by the server. 