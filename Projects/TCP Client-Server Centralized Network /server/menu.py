...
class Menu(object):
    ...
    def process_user_data(self):
        ...
        if 1 <= option <= 6:# validates a valid option
            ...
            functionName = "option"+str(option)
            # functionData = {"option_selected":int(getattr(self,functionName)()),"clientid": self.client.get_client_id()}
            functionData = getattr(self,functionName)()

            if("message" in functionData):
               data.update({"message":functionData["message"]})
            if("room_id" in functionData):
               data.update({"room_id":functionData["room_id"]})
            if('recipientId'in functionData):
                data.update({"recipient_id":functionData['recipientId']})
            data.update({"option_selected":int(functionData["option"]),"clientid": self.client.get_client_id()})
            print(data)
            self.client.send(data)
    ...
    def option_selected(self):
        ...
        # TODO: your code here.
        try:
            option = int(input(self.get_menu()+"\nYour option <enter a number>: "))
    ...
    def option2(self):
        ...
        recipientId = int(input("Enter recipient id: "))
        
        data.update({"message":message+"(from: "+self.client.idKey+")","recipientId":recipientId})
        return data
    ...
    def option4(self):
        ...
        data['option'] = 4
        # Your code here.
        room_id = input("Create a chat room\nEnter room id: ")
    ...
    def option5(self):
        ...
        data['option'] = 5
        # Your code here.
        room_id = input("Join a chat room\nEnter room id: ")
        data.update({"room_id":room_id})
        return data
