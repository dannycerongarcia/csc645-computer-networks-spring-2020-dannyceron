...
* General description of the project (a few sentences)
    the goal of the project is to understand tcp connection by inplementing a server which connects to clients, the server send an onbject to the client,
    the the server creates a client handler thread so that it can comminicate to the object sent to the client. the onjective is to have the client 
    communicate to the client handler thread so that it can proccess it request instead of the server, because we dont want to overload the server main thread
...
* Attach screenshots or videos to this file to ilustrate how your program works for all the options in the menu. 
    name of the file is TCPtest.gif
    ![alt text](TCPtest.gif)
* A few sentences about all the challenges you found during the implementation of this project and how you overcame them. Please be honest here. 
    my main challege was find a way to send an object to the client that would work but since the client did not have the file already installed in their machine.
    It made it impossible for me to do it. so i manually just put the file in the client machine
    another chanllege at 1st were that the client sometimes would not get the menu object. but i decided to just make a single send for that instead sending it while in a loop.
    the last challange was the chat room i could not auto matically send\refresh the data real time on the client side. 
...
link: <https://github.com/sfsu-joseo/csc645-computer-networks-spring-2020-dannyceron94/tree/master/Projects/TCP%20Client-Server%20Centralized%20Network%20>

<Danny Daneth Ceron Garcia>
<your student id: 918581149>
<your github username: dannyceron94>
