...
# Student Name: Danny Daneth Ceron Garcia
...
class Server(object):
    ...
    def _handler(self, clienthandler):
        ...
        while True:
            ...
            #  lot of the logic will happen here
            data = self.receive(clienthandler = clienthandler)
            ...
            if(not data):
             ...
             # TODO: Otherwise, send acknowledge to client. (i.e a message saying 'server got the data
                ...
                data = {"acknowledge":"server got the data"}
    ...
    def _accept_clients(self):
        ...
        while True:
            try:
               ...
               server_ip = str(addr[0]) + "/" + str(self.port)
               client_id = addr[1]
               # TODO: send assigned id to the new client. hint: call the send_clientid(..) method
               self._send_clientid(clienthandler,addr)
               # receive, process, send response to client.
            #    handle multiple cliets trying to conect
               Thread(target=self._handler,args=(clienthandler)).start()
            except:
    ...
    def _send_clientid(self, clienthandler, clientid):
        ...
        :return: VOID
        """
        server_ip = str(clientid[0]) + "/" + str(self.port)
    ...
    def receive(self, clienthandler, MAX_ALLOC_MEM=4096):
        ...
        """
        return pickle.loads(clienthandler.recv(MAX_ALLOC_MEM))
